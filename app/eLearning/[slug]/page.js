...
export default async function CourseDetails({ params }) {
  ...
  const eLearningContents = eLearningCoursesContents.filter(
    (item) => item?.eLearningDataId === courseDesc?.id
  ); // Use 'slug' here
  ...
  return (
    <Layout user={user}>
      <BreadcrumbSection
        header="E-Learning Course Details"
        title="E-Learning Details"
      />
      {courseDesc ? (
        <ELearningDetailSection
          courseData={courseDesc}
          courseContents={eLearningContents}
          user={user}
        />
      ) : (
